<script lang="ts">
    ...
    const buildTimeline = () => {
        ...
        timeline
            .to('.nav-inner', { y: '0%', duration: 0.2 }, 0)
            .to(cross, {
                rotation: 45,
                opacity: 0.5,
                expanded: true,
...
<header class="fixed mt-8 mr-4 md:m-8 right-0 z-200">
    <button
        bind:this={toggleBtn}
        type="button"
...
<dialog
    ...
    class="z-100 m-0 p-0 w-full h-dvh bg-transparent overflow-hidden"
>
    <div class="
        nav-inner
        ...
        <nav aria-label="Pages">
            ...
            {#each pages as page, index (page.name)}
                <li class="page-link md:py-4">
                    <a class="page-link__anchor inline-flex items-baseline gap-4"
                        href={page.href}
                        onclick={ (e) => {
                            e.preventDefault();
                            navigate(page.href);
...
<style>
    ...
    /* Dialog is transparent — background shows through around the panel */
    #nav-overlay {
        position: fixed;
        inset: 0;
        width: 100%;
    ...
    /* Vertical rule overlay on the panel */
    .nav-inner::before {
        content: '';
        position: absolute;
        inset: 0;
